<?php if(empty($_SERVER["PATH_INFO"])) exit;
	...
	if(count($get) == 2){
		...
		// http://www.demonoid.pw/files/details/xxx/yyy
		$details = "/files/details/{$get[0]}/{$get[1]}/";
		$page = new DOMDocument;
		@$page->loadHTML(file_get_contents("http://www.demonoid.pw/$details"));
		$XPath = new DOMXPath($page);
		$tables = $XPath->query('//table/tr/td[@class="ctable_content"]/a');
		$torrent = NULL;
		for($i = 0; $i < $tables->length; $i++){
			// Torrent links are:
			// http://www.demonoid.pw/files/download/xxx/yyy
		...
		if(!is_null($torrent)){
			// POST to https://www.demonoid.pw/account_handler.php
			...
			curl_setopt_array($ch, array(
				CURLOPT_URL => 'http://www.demonoid.pw/account_handler.php',
				CURLOPT_POST => true,
		...
		$rss = new DOMDocument();
		$rss->load("http://www.demonoid.pw/rss/users/{$get[0]}.xml");
